...
namespace StarRail_Launcher.ViewModels
{
    ...
    public class SettingsPageViewModel : ObservableObject
    {
        ...
        {
            string CGamePath = Path.Combine(GamePath, "StarRail.exe");
            // string GlobalGamePath = Path.Combine(GamePath, "GenshinImpact.exe");
            if (GamePath != "" && File.Exists(CGamePath))
            {
        ...
        private async Task GameFileConvert()
        {
            ...
            if (!fileHelper.IsFileOpen(Path.Combine(App.Current.DataModel.GamePath, "StarRail.exe")))
            {
