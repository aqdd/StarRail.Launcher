...
namespace StarRail_Launcher.Service
{
    ...
    {
        private const string VersionUrl = "https://api-launcher-static.mihoyo.com/hkrpg_cn/mdk/launcher/api/resource?channel_id=1&key=6KcVuOkbcqjJomjZ&launcher_id=33&sub_channel_id=1";
        /// <summary>
        /// 更新服务的主方法
        /// </summary>
        /// <param name="vm"></param>
        ...
        public async void CheckUpdate(MainWindow main)
        {
            ...
            try
            {
                string versionInfo = await new HttpClient().GetStringAsync(VersionUrl);
                var o = JsonConvert.DeserializeObject<JObject>(versionInfo);
                var v = o["data"]["game"]["latest"]["version"];
                var ver = v.Value<string>();
                if (ver != "")
                {
                    App.Current.UpdateObject.PkgVersion = $"{ver}-1";
                }
            }
            catch (Exception e)
            {
                MessageBox.Show("程序运行出问题了");
            }
