...
namespace StarRail_Launcher.Service
{
    ...
    public class MainService : IMainWindowService
    {
        ...
        public async void LoadQQGroups(MainWindowViewModel vm)
        {
            string info = await HtmlHelper.GetInfoFromHtmlAsync("QQGroups");
            QQGroup qqGroup = JsonConvert.DeserializeObject<QQGroup>(info);
            if (qqGroup==null)
            {
                return;
            }

            vm.QQGroups = qqGroup.Groups;
        }
        ...
        public void CheckConfig(MainWindow main)
        {
            ...
            if (!File.Exists(Path.Combine(App.Current.DataModel.GamePath ?? "Err", "StarRail.exe")))
            {
